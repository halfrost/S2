...
func (cu *CellUnion) Normalize() {
	...
	for _, ci := range *cu {
		...
		// 如果前一个 cell 包含当前的 cell，那么就忽略掉当前的。我们只需要去检查最后的一个 cell。
		// 因为 cells 数组是有序的，并且不会是逆序的，而且输出也不会冗余，所以候选的 cell 不被最后一个 cell 所包含，那么它也一定不会被先前的 cell 所包含。
...
func (cu *CellUnion) Denormalize(minLevel, levelMod int) {
	fmt.Printf("初始参数 minLevel = %d levelMod = %d\n", minLevel, levelMod)
	var denorm CellUnion
	for _, id := range *cu {
		level := id.Level()
		fmt.Printf("初始的 level = %d\n", level)
		newLevel := level
